...
Processes multiple dialogue files and cycles through background videos
"""
...
def process_dialogue(dialogue_file: Path, background_video: str, output_name: str):
    """Process a single dialogue file into a video"""
    ...
    print(f"📹 Background: {Path(background_video).name}")
...
def batch_process_dialogues():
    """Process all dialogue files with cycling backgrounds"""
    
    base_dir = Path(__file__).parent
    dialogues_dir = base_dir / "inputs" / "dialogues"
    assets_dir = base_dir / "inputs" / "assets"
    
    # Get all dialogue files
    dialogue_files = list(dialogues_dir.glob("*.json"))
    dialogue_files.sort()  # Process in alphabetical order
    
    # Get all background videos from assets/backgrounds folder
    backgrounds_dir = assets_dir / "backgrounds"
    background_videos = []
    for ext in ["*.mp4", "*.MP4"]:
        background_videos.extend(backgrounds_dir.glob(ext))
    background_videos.sort()
    
    print(f"🎯 Found {len(dialogue_files)} dialogue files")
    print(f"📹 Found {len(background_videos)} background videos")
    
    if not dialogue_files:
        print("❌ No dialogue files found in inputs/dialogues/")
        return
    
    if not background_videos:
        print("❌ No background videos found in inputs/assets/backgrounds/")
        return
    
    print(f"\n🚀 Starting batch processing...")
    
    # Process each dialogue with cycling backgrounds
    for i, dialogue_file in enumerate(dialogue_files):
        ...
        background_name = background_video.stem
        output_name = f"{dialogue_name}_{background_name}.mp4"
        
        try:
            process_dialogue(dialogue_file, str(background_video), output_name)
        except Exception as e:
            print(f"❌ Error processing {dialogue_file.name}: {str(e)}")
            continue

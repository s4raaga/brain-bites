...
def _inject_stored_web_storage(page: Page, base_url: str, profile_dir: str) -> None:
	...
	if not ls_data and not ss_data:
		return
	lines: List[str] = ["try {", f"  if (location.host.includes('{host}')) {{"]
		...
		if attempt >= fallback_navigate_after and not fallback_done and cur_host == base_host:
			...
			if not page.url.rstrip('/').endswith('/ultra/course'):
				with contextlib.suppress(Exception):
					...
					time.sleep(2)
		time.sleep(1)
	raise TimeoutError("Login not detected within timeout. If your SSO uses MFA, keep the window focused and retry.")
...
@mcp.tool("login", description=(
	"Establish / refresh authenticated Blackboard Chromium session (persisted profile dir). "
	"DO NOT call at start of a session. First try list_courses. Only call login if another tool's"
	" response explicitly includes 'authenticated': false or if list_courses returns 0 courses AND"
	" auth_indicators suggests a login page. Base URL hard-coded."
))
	...
	def _do():
		...
		with launch_context(headless=headless, profile_dir=PROFILE_DIR) as ctx:
			...
			try:
				...
				pass
		return {"profile_dir": os.path.abspath(PROFILE_DIR), "logs": logs, "host_cookie_summary": cookie_brief}
	return _run_in_thread(_do)

@mcp.tool("list_courses", description=(
	"List available courses (first step – NEVER call login beforehand). Returns courses plus an"
	" 'authenticated' boolean. If authenticated is false, then and only then consider calling"
	" the login tool. Base URL hard-coded."
))
def tool_list_courses(headless: bool = False) -> dict:
	def _do():
		...
		with launch_context(headless=headless, profile_dir=PROFILE_DIR) as ctx:
			...
			# Determine probable authentication status: if we have at least one course, assume authenticated.
			low_html = html.lower()
			login_markers = ["login", "password", "sign in", "idp", "single sign-on"]
			marker_hits = [m for m in login_markers if m in low_html]
			authenticated = bool(courses)
			# Heuristic: some Blackboard landing pages may contain the word 'login' even when authenticated;
			# only treat as unauthenticated if NO courses AND multiple classic auth markers.
			if not authenticated and len(marker_hits) >= 2:
				authenticated = False
			else:
				# If ambiguous (no courses but few markers) leave authenticated False but provide markers.
				pass
			if not courses:
				with contextlib.suppress(Exception):
					page.goto(direct_ultra, wait_until="domcontentloaded")
			...
			return {
				...
				"auth_indicators": marker_hits[:10],
				"debug": {"current_url": page.url, "html_chars": len(html), "dump_path": debug_path},
			}
	return _run_in_thread(_do)
	...
	def _do():
		...
		with launch_context(headless=headless, profile_dir=PROFILE_DIR) as ctx:
			...
			for u in selected:
				...
				try:
					resp = client.get(u, timeout=DEFAULT_TIMEOUT/1000 * 3)
					if resp.ok:
						data = resp.body()
						if not data:
		...
		result: Dict[str, object] = {"saved_count": len([s for s in saved if "error" not in s]), "files": saved, "out_dir": DOWNLOAD_DIR}
		if deprecated_used:
...
def resource_downloads_index() -> dict:
	...
	for entry in sorted(os.listdir(DOWNLOAD_DIR)):
		...
		try:
			st = os.stat(path)
			entries.append({"name": entry, "bytes": st.st_size, "mtime": int(st.st_mtime), "path": path})
		except Exception:
			pass
	return {"directory": DOWNLOAD_DIR, "count": len(entries), "files": entries}
...
@mcp.tool("save_json", description="Persist a grounded video script JSON. Fields: title (course code), description (1-2 sentences citing topics covered), dialogue (list of {character,text}). characters allowed: Speaker A, Speaker B. Style rotate: dialog, mentor->learner, monologue, anecdote. Only call AFTER sufficient read_pdf_text evidence (≥400 chars per covered course/topic). No hallucinations.")
def tool_save_json(filename: str, data: Union[Dict[str, Any], List[Any], str], overwrite: bool = False, pretty: bool = True) -> dict:  # type: ignore[valid-type]
	...
	return {"saved": True, "filename": os.path.basename(target), "path": os.path.abspath(target), "bytes": size, "object_type": type(obj).__name__, "keys": top_keys}
...
def tool_read_pdf_text(name: str, max_pages: int = 8, max_chars: int = 10_000) -> dict:
	...
	return {"ok": True, "name": name, "pages_parsed": pages, "chars": len(joined), "text": joined}
